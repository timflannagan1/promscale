...
func TestMigrateLock(t *testing.T) {
	...
	withDB(t, *testDatabase, func(db *pgxpool.Pool, _ testing.TB) {
		...
		reader, err := timescale_prometheus.CreateClient(&cfg, metrics)
		...
		cfg2 := cfg
		cfg2.Migrate = true
		migrator, err := timescale_prometheus.CreateClient(&cfg2, metrics)
		...
		_, err = timescale_prometheus.CreateClient(&cfg3, metrics)
		if err == nil {
		...
		only_migrator, err := timescale_prometheus.CreateClient(&cfg3, metrics)
		if err != nil {
			t.Fatal(err)
		}
